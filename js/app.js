...
function createBookCard(book){
    return `
        <article class="overflow-hidden rounded-2xl border border-emerald-900/50 bg-white dark:bg-zinc-900 transition duration-300 hover:-translate-y-1 hover:border-emerald-700">
            <img src="${book.image}" alt="${book.title} Cover" class="h-56 w-full object-cover">
            <div class="p-5">
                <div class="mb-4">
                    <h3 class="mb-1 text-xl font-bold text-zinc-900 dark:text-zinc-100">${book.title}</h3>
                ...
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <span class="rounded-full bg-zinc-200 px-3 py-1 text-sm text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"> ${book.status} </span>                    
                </div>
            </div>
        </article>
    `;
